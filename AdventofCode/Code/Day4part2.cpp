...
namespace Day4 {
    ...
    struct Bingo{
        ...
        std::vector<bool> boardsHaveBingo;
        int numBoardsBingo = 0;
        int current_number;
        void addBall(int number);
        void addNumbertoBoards(int number);
        void checkBingo(int i, int j, int k);
    ...
    void Day4::Bingo::addNumbertoBoards(int num){
        ...
        if(lastBoardNum == -1){
            ...
            boardsHaveBingo.push_back(false);
        } else {
    ...
    void Day4::Bingo::checkBingo(int i, int j, int k){
        int rows = 0;
        int columns = 0;
        for(int p = 0; p<5;p++){
        ...
        if((rows==5||columns==5)&&!boardsHaveBingo[i]){
            numBoardsBingo++;
            boardsHaveBingo[i]=true;
        }
    }

    bool Day4::Bingo::callBall(int num){
        for(int i=0;i<boards.size();i++){
            if(boardsHaveBingo[i]) continue;
            for(int j=0;j<boards[i].size();j++){
                for(int k=0;k<boards[i][j].size();k++){
                    if(boards[i][j][k].number == num){
                        boards[i][j][k].called = true;
                        checkBingo(i,j,k);
                        if(numBoardsBingo == boards.size()){
